...
def test_export_csv_three_columns_with_module(tmp_path):
    ...
    assert stats.qa_rows == 1                             # source-faithful main query only
...
def test_export_expanded_variant_is_separate_from_primary(tmp_path):
    """The primary artifact is source-faithful; expansion is opt-in and separate."""
    import csv
    u = QAUnit(query="任务OOM怎么办", answer="加内存",
               sources=[Provenance(topic="01_模型训练常见问题自查")])
    u.paraphrases = ["内存爆了咋整", "显存不够怎么处理"]
    # Experimental expansion: 1 main + 2 paraphrases = 3 rows.
    full = export_all([u], [], tmp_path, include_paraphrases=True)
    assert full.qa_rows == 3
    # Primary export remains one row and does not clobber the experiment.
    primary = export_all([u], [], tmp_path, include_paraphrases=False)
    assert primary.qa_rows == 1
    default_csv = tmp_path / "qa_pairs.csv"
    expanded_csv = tmp_path / "qa_pairs_with_paraphrase.csv"
    assert default_csv.is_file() and expanded_csv.is_file()
    primary_rows = list(csv.reader(open(default_csv, encoding="utf-8-sig")))
    expanded_rows = list(csv.reader(open(expanded_csv, encoding="utf-8-sig")))
    assert len(expanded_rows) == 4 and len(primary_rows) == 2
    assert primary_rows[0] == ["Query", "Answer", "Module"]
    assert primary_rows[1][0] == "任务OOM怎么办"
    assert "内存爆了咋整" not in {r[0] for r in primary_rows}
    ...
    # Experimental zip is distinct; primary zip remains the production artifact.
    assert (tmp_path / "知识库上传包.zip").is_file()
    assert (tmp_path / "知识库上传包_含扩写.zip").is_file()
